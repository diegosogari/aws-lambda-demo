...
class TestHandleAuth(unittest.TestCase):
    ...
    def test_without_token(self):
        self.assertIsNone(handler.handle_auth({}))

    def test_with_good_accesstoken(self):
        self.assertEqual(handler.handle_auth({
    ...
    def test_with_bad_accesstoken(self):
        with self.assertRaises(Exception):
            handler.handle_auth({"x-amzn-oidc-accesstoken":""})
            
    def test_with_bad_userclaims(self):
        with self.assertRaises(Exception):
            handler.handle_auth({"x-amzn-oidc-data":""})
